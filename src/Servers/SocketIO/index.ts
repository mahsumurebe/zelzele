...
export default class WS implements IServer {
    ...
    constructor() {
        this.config = {
            hostname: process.env.SOCKETIO_SERVER_HOSTNAME,
            port: +(process.env.SOCKETIO_SERVER_PORT || '3001'),
            https: {
                use: process.env.SOCKETIO_SERVER_HTTPS_USE === 'true',
                certFile: process.env.SOCKETIO_SERVER_HTTPS_CERT_FILE,
                privateKey: process.env.SOCKETIO_SERVER_HTTPS_PRIVATE_KEY_FILE,
            },
        };
